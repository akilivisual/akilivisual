...
export function GalleryModule({ module }: { module: ModuleWithActors }) {
  ...
  const transitionType = (props.transition_type as string) ?? 'fade'
  ...
  if (transitionType === 'slide') {
    return (
      <div className="absolute inset-0 overflow-hidden" style={{ pointerEvents: 'auto' }}>
        ...
        >
          {images.map((src, i) => (
            <div key={i} style={{ width: `${100 / count}%`, height: '100%', flexShrink: 0, position: 'relative' }}>
  ...
  // fade (default) — all images mounted, pure opacity crossfade
  return (
    <div className="absolute inset-0 overflow-hidden" style={{ pointerEvents: 'auto' }}>
      {images.map((src, i) => (
        <div
